<template>
    <div class="surat-premium"
        :class="{
            ...
            'memorisation-sidebar-open': isMemorisationOffcanvasVisible,
                    ...
                    <section
                        ...
                        <div v-if="!continueReadingCardMinimized" class="continue-reading-card-body">
                            <div class="continue-reading-card-copy">
                                ...
                                <div class="continue-reading-card-status">
                                    <p v-if="continueReadingUpdatedLabel" class="continue-reading-card-updated mb-0">
                                        Saved {{ continueReadingUpdatedLabel }}
                                    </p>
                                    <p class="continue-reading-card-hint mb-0">
                                        <i class="bi bi-signpost-2" aria-hidden="true"></i>
                                        <span>
                                            {{ isContinueReadingResuming ? 'Opening saved ayah...' : 'Play jumps back to this saved point' }}
                                        </span>
                                    </p>
                                </div>
                            </div>
                            <div
                                ...
                                <button
                                    ...
                                    :disabled="isContinueReadingResuming"
                                    :aria-busy="isContinueReadingResuming ? 'true' : 'false'"
                                    @click="resumeContinueReading"
                                    :aria-label="`Resume from ${continueReadingSurahLabel}, ${continueReadingAyahLabel}`"
                                    title="Resume">
                                    <i
                                        class="bi"
                                        :class="isContinueReadingResuming ? 'bi-arrow-repeat continue-reading-card-spinner' : 'bi-play-fill'"
                                        aria-hidden="true"></i>
                                </button>
                                <button
                                    type="button"
                        ...
                        <div v-else class="continue-reading-card-minimized">
                            <div class="continue-reading-card-minimized-main">
                                ...
                                <span>
                                    {{ isContinueReadingResuming ? 'Opening saved ayah...' : continueReadingAyahLabel }}
                                </span>
                            </div>
        ...
        <teleport to="body">
            <transition name="countdown-fade">
                <div
                    v-if="memorisationStartCountdownRemaining > 0"
                    class="memorisation-start-countdown"
                    role="dialog"
                    aria-modal="true"
                    aria-live="assertive"
                    aria-atomic="true"
                    aria-label="Memorisation countdown">
                    <div class="memorisation-start-countdown-card">
                        <span class="memorisation-start-countdown-label">{{ memorisationStartCountdownLabel || 'Starting' }}</span>
                        <strong>{{ memorisationStartCountdownRemaining }}</strong>
                        <small>{{ isMemorisationChainingActive ? 'Get ready for the next link.' : 'Get ready to recite.' }}</small>
                    </div>
                </div>
            </transition>
        </teleport>
            ...
            <div class="col-12 col-xl-10">
                ...
                <template v-else>
                        ...
                        <div
                            ...
                            <div v-if="!isMemorisationToolbarVisible" class="advanced-quran-mobile-select-shell">
                                <select
	                            ...
	                            <div v-if="isMemorisationToolbarVisible" class="memorisation-mobile-main-surah-name" role="status" aria-live="polite">
	                                <strong>{{ desktopSurahContext.englishName }}</strong>
	                                <span v-if="desktopSurahContext.arabicName" dir="rtl">{{ desktopSurahContext.arabicName }}</span>
	                            </div>
                                <div v-if="isMemorisationToolbarVisible" class="dropdown memorisation-mobile-tools-dropdown">
                                    <button
                                        type="button"
                                        class="btn advanced-quran-mobile-icon-btn memorisation-mobile-tools-dropdown-btn dropdown-toggle"
                                        data-bs-toggle="dropdown"
                                        aria-expanded="false"
                                        aria-label="Open memorisation tools menu"
                                        title="Memorisation tools">
                                        <i class="bi bi-sliders2" aria-hidden="true"></i>
                                    </button>
                                    <div class="dropdown-menu dropdown-menu-end advanced-quran-more-menu memorisation-mobile-tools-menu">
                                        <button
                                            v-for="tool in memorisationBeginnerToolOptions"
                                            :key="`memorisation-mobile-main-menu-${tool.key}`"
                                            type="button"
                                        ...
                                        <button
                                            ...
                                            <i class="bi" :class="isMemorisationOffcanvasVisible ? 'bi-layout-sidebar-inset' : 'bi-layout-sidebar-inset-reverse'" aria-hidden="true"></i>
                                            <span class="advanced-quran-more-item-label">{{ isMemorisationOffcanvasVisible ? "Close Tools Sidebar" : "Open Tools Sidebar" }}</span>
                                        </button>
                                    </div>
                                </div>
		                        <button
                        ...
                        <div
                            ...
                            <div v-if="isMemorisationToolbarVisible && !isMemorisationOffcanvasVisible" class="memorisation-mobile-quick-panel memorisation-mobile-quick-panel--beginner">
                                <div class="memorisation-mobile-surah-title">
                                    <span class="memorisation-toolbar-trust-pill">Beginner friendly</span>
                                    <strong>{{ desktopSurahContext.englishName }}</strong>
                                    <span v-if="desktopSurahContext.arabicName" dir="rtl">{{ desktopSurahContext.arabicName }}</span>
                                </div>
                                <div class="memorisation-mobile-flow-grid">
                                ...
                                <div class="memorisation-mobile-tools-strip" role="group" aria-label="Beginner memorisation tools">
                                    <button
                                        v-for="tool in memorisationPrimaryToolOptions"
                                        :key="`memorisation-mobile-tool-${tool.key}`"
                                        type="button"
                                    ...
                                    <div class="dropdown memorisation-toolbar-overflow">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end advanced-quran-more-menu memorisation-toolbar-overflow-menu">
                                            <button
                                                ...
                                                :key="`memorisation-mobile-overflow-${tool.key}`"
                                                type="button"
                                                class="dropdown-item advanced-quran-more-item memorisation-toolbar-overflow-item"
                                                :class="{ 'is-enabled': tool.active }"
                                                @click="handleMemorisationBeginnerTool(tool.key)">
                                            ...
                                            <button
                                                ...
                                                <span class="advanced-quran-more-item-label">Open Tools Sidebar</span>
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div
                                    ...
                                    <button
                                        type="button"
                                        class="btn memorisation-onboarding-close"
                                        @click="dismissMemorisationOnboarding"
                                        aria-label="Close memorisation welcome">
                                        <i class="bi bi-x-lg" aria-hidden="true"></i>
                                    </button>
                                    <span class="memorisation-toolbar-trust-pill">Beginner friendly</span>
                                    <h3>Memorise Qur'an بسهولة</h3>
                                    <p>
                                        Build a small daily connection with the Qur'an. Select a Surah, adjust the range and tools, then start.
                                    </p>
                                    <div class="memorisation-onboarding-steps" aria-hidden="true">
                                        <span>Surah</span>
                                    ...
                                    <button
                                        type="button"
                                        class="btn memorisation-onboarding-primary"
                                        @click="startMemorisationOnboardingWithAlFatiha">
                            ...
                            <div class="advanced-quran-mobile-action-grid">
                                <button
                                    ...
                                    title="Sessions">
	                                    ...
	                                    <span class="advanced-quran-mobile-action-label">Sessions</span>
	                                    <span class="advanced-quran-mobile-action-btn-state">Open</span>
	                                </button>
                                <button
                                    v-if="!isReaderToolbarMinimized"
                                    type="button"
                                ...
                                <button
                                    ...
                                    type="button"
                                    class="btn advanced-quran-mobile-action-btn"
                                    :class="{ 'is-enabled': showWordTranslation && showWordTranslationTooltip }"
                                    @click="toggleToolbarWordMeaningWithTooltip"
                                    aria-label="Toggle word meaning with tooltip"
                                    title="Show or hide word meaning with tooltip.">
                                    <i class="bi bi-translate" aria-hidden="true"></i>
                                    <span class="advanced-quran-mobile-action-label">Word meaning</span>
                                    <span class="advanced-quran-mobile-action-btn-state">
                                        {{ showWordTranslation && showWordTranslationTooltip ? "On" : "Off" }}
                                    </span>
                                </button>
                                <button
                                    v-if="!isReaderToolbarMinimized"
        ...
        <div v-if="(surahDetails || currentSurahInfo) && (!isTabletOrMobile && showDesktopToolbar && showReaderToolbar) && !isReadingFullscreen"
            ...
            <div
                ...
                <div
                    v-if="isMemorisationToolbarVisible && !isMemorisationModeActive && !isReaderToolbarMinimized && isMemorisationOffcanvasVisible"
                    class="memorisation-toolbar-layout memorisation-toolbar-layout--beginner">
                    <div class="memorisation-toolbar-intro memorisation-toolbar-intro--beginner">
                        <div class="memorisation-toolbar-surah-name">
                            <span class="memorisation-toolbar-trust-pill">Beginner friendly</span>
                            <strong>{{ desktopSurahContext.englishName }}</strong>
                            <span v-if="desktopSurahContext.arabicName" class="memorisation-toolbar-surah-arabic" dir="rtl">
                                {{ desktopSurahContext.arabicName }}
                            </span>
                        </div>
	                ...
	                <div
	                    v-if="!isMemorisationToolbarVisible || isMemorisationModeActive || !isReaderToolbarMinimized"
                        ...
                        <button
                            ...
                            <span class="quran-toolbar-btn-main">
                                ...
                                <span class="quran-toolbar-btn-copy">
                                    <span class="quran-toolbar-btn-text">Word meaning</span>
                                </span>
                            </span>
		                            <span class="quran-toolbar-btn-state">{{ showWordTranslation && showWordTranslationTooltip ? "On" : "Off" }}</span>
		                        </button>
                            ...
                            <span class="quran-toolbar-btn-main">
                                ...
                                <span class="quran-toolbar-btn-copy">
                                    <span class="quran-toolbar-btn-text">Tooltip</span>
                                </span>
                            </span>
                            <span class="quran-toolbar-btn-state">{{ showWordTranslationTooltip ? "On" : "Off" }}</span>
                        </button>
                        ...
                        <button
                            ...
                            :title="isMemorisationOffcanvasVisible ? 'Close tools sidebar' : 'Open tools sidebar'">
                            <span class="quran-toolbar-btn-icon-shell" aria-hidden="true">
                                <i
                                    class="bi"
		                        ...
		                        <button
		                            ...
		                            <span class="memo-kbd-popover" role="tooltip" aria-hidden="true">
		                                <strong>Keyboard shortcuts</strong>
		                                <span>Space: Play/Pause</span>
		                                <span>←/→: Prev/Next ayah</span>
		                                <span>B: Bookmark</span>
		                                <span>R: Reflect</span>
		                                <span>L: Repeat range</span>
		                            </span>
		                        </button>
		                    </div>
	                    <button
	                        ...
	                        <i class="bi bi-x-lg" aria-hidden="true"></i>
		                        ...
		                        <div
		                            ...
		                            aria-live="polite"
		                            aria-atomic="true">
                                    <div class="memorisation-toolbar-progress-main">
                                        <div class="memorisation-toolbar-progress-copy">
                                        ...
                                        <div class="memorisation-toolbar-progress-actions" role="group" aria-label="Memorisation playback controls">
                                            <span class="memorisation-toolbar-progress-range-label">
                                            ...
                                            <button
                                                ...
                                                :class="{
                                                    ...
                                                    'is-resume': !isMemorisationModeActive && hasMemorisationResumeCandidate()
                                                }"
                                                @click="onMemorisationProgressPlayClick"
                                                :aria-label="isAnyAudioPlaying
                                                    ? 'Pause memorisation session'
                                                    : (!isMemorisationModeActive && hasMemorisationResumeCandidate()
                                                        ? 'Continue from your latest saved memorisation session'
                                                        : 'Play memorisation session')"
                                                :title="isAnyAudioPlaying
                                                    ? 'Pause'
                                                    : (!isMemorisationModeActive && hasMemorisationResumeCandidate()
                                                        ? 'Continue'
                                                        : 'Play')">
                                                <i class="bi" :class="isAnyAudioPlaying ? 'bi-pause-fill' : 'bi-play-fill'" aria-hidden="true"></i>
                                                <span class="memorisation-toolbar-progress-play-label">
                                                    {{
                                                        isAnyAudioPlaying
                                                            ? 'Pause'
                                                            : (!isMemorisationModeActive && hasMemorisationResumeCandidate()
            ...
            <button
	            ...
	            <div class="offcanvas-body">
                    ...
                    <button
                        ...
                        :class="{ 'is-active': isMemorisationAdvancedMode }"
                        :aria-selected="isMemorisationAdvancedMode ? 'true' : 'false'"
                        @click="setMemorisationToolsDepth('advanced')">
                        Advanced
                    </button>
                </div>
                <section
                        ...
                        <div v-show="isMemorisationBeginnerSectionOpen('setup')" class="memorisation-tools-grid">
                            <label class="memorisation-tools-field memorisation-tools-field--full">
                                ...
                                <select
                                    ...
                                    @change="onMemorisationToolbarSurahChange"
                                    aria-label="Select surah">
                                    <option v-for="surah in filteredSurahs" :key="`beginner-surah-${surah.number}`" :value="String(surah.number)">
                                        {{ surah.number }}. {{ surah.englishName }}
                                    </option>
                                </select>
                                <small class="memorisation-tools-field-hint">Choose the chapter.</small>
                            </label>
                            ...
                            <label class="memorisation-tools-field memorisation-tools-field--full">
                                ...
                                <select
                                    ...
                                    <option v-for="reciter in recitersSorted" :key="`beginner-reciter-${reciter.identifier}`" :value="reciter.identifier">
                                        {{ reciter.englishName }}
                                    </option>
                                </select>
                            ...
                            <label class="memorisation-tools-field memorisation-tools-field--full">
                                ...
                                <div class="memorisation-tools-range">
                                    <input
                                        type="number"
                                    ...
                                    <span class="memorisation-tools-range-sep">to</span>
                                    <input
                                        type="number"
                                        class="form-control memorisation-tools-control"
                                        v-model.number="memorisationRangeEnd"
                        ...
                        <div v-show="isMemorisationBeginnerSectionOpen('playback')" class="memorisation-tools-grid">
                            <label class="memorisation-tools-field">
                                ...
                                <select
                                    ...
                                    <option v-for="speed in playbackSpeeds" :key="`beginner-speed-${speed}`" :value="Number(speed)">
                                        {{ Number(speed) }}x
                                    </option>
                                </select>
                                <small class="memorisation-tools-field-hint">Slow down or keep normal speed.</small>
                            </label>
                            ...
                            <label class="memorisation-tools-field">
                                ...
                                <small class="memorisation-tools-field-hint">Adds thinking time between ayahs for active recall before the next verse.</small>
                            </label>
                            ...
                            <label class="memorisation-tools-field">
                                ...
                                <small class="memorisation-tools-field-hint">Higher repetition strengthens short-term encoding into longer-term retention.</small>
                            </label>
                            ...
                            <div
                                ...
                                <div class="memorisation-waveform-bars" aria-hidden="true">
                                    <i v-for="bar in 14" :key="`memo-wave-${bar}`"></i>
                                </div>
                            </div>
                        ...
                        <div class="memorisation-tools-grid">
                            <label class="memorisation-tools-switch memorisation-tools-field--full">
                                <span class="memorisation-tools-switch-copy">
                                    ...
                                    <small>Softens the next ayah so your brain retrieves from memory first, then verifies.</small>
                                    <span class="memorisation-blur-preview" aria-hidden="true">
                                        <span></span>
                                    </span>
                                </span>
		                            ...
		                            <label class="memorisation-tools-field memorisation-tools-field--full">
		                                ...
		                                <select
		                                    ...
		                                    <option
		                                        ...
		                                        :key="`memo-session-beginner-${entry.id}`"
                    ...
                    <section class="memorisation-tools-card" aria-label="Advanced setup">
                        ...
                        <div class="memorisation-tools-card-head">
                                ...
                                <button
                                    type="button"
                                    class="btn memorisation-section-toggle"
                                    @click="toggleMemorisationAdvancedSection('setup')"
                                    :aria-expanded="isMemorisationAdvancedSectionOpen('setup') ? 'true' : 'false'"
                                    aria-label="Toggle setup section">
                                    <i class="bi" :class="isMemorisationAdvancedSectionOpen('setup') ? 'bi-chevron-up' : 'bi-chevron-down'" aria-hidden="true"></i>
                                </button>
	                        </div>
	                        <div v-show="isMemorisationAdvancedSectionOpen('setup')" class="memorisation-tools-grid memorisation-advanced-section-body">
                            <label class="memorisation-tools-field memorisation-tools-field--full">
                                <span class="memorisation-tools-label">Surah name</span>
                                <select
                                    class="form-select memorisation-tools-control"
                            ...
                            <label class="memorisation-tools-switch memorisation-tools-field--full">
                                <span class="memorisation-tools-switch-copy">
                                    <strong>Blur ayah</strong>
                                    <small>Blur previous and next ayah while keeping current ayah clear for recall.</small>
                                    <span class="memorisation-blur-preview" aria-hidden="true">
                                        <span></span>
                                    </span>
                                </span>
                            ...
                            <label class="memorisation-tools-switch memorisation-tools-field--full">
                                <span class="memorisation-tools-switch-copy">
                                    <strong>Save session</strong>
                                    <small>Save completed practice locally and reload it later from Saved session.</small>
                                </span>
                                <span class="form-check form-switch mb-0">
                                    <input
                                        class="form-check-input"
                    ...
                    <section class="memorisation-tools-card"
                        ...
                        <div class="memorisation-tools-card-head">
                                ...
                                <button
                                    type="button"
                                    class="btn memorisation-section-toggle"
                                    @click="toggleMemorisationAdvancedSection('chaining')"
                                    :aria-expanded="isMemorisationAdvancedSectionOpen('chaining') ? 'true' : 'false'"
                                    aria-label="Toggle chaining section">
                            ...
                            <label class="memorisation-tools-switch memorisation-tools-field--full">
                                ...
                                <span class="form-check form-switch mb-0">
                                    <input
                                        ...
                                        aria-label="Toggle verse countdown" />
                                </span>
                            </label>
                        </div>
                    </section>
                    ...
                    <section class="memorisation-tools-card" aria-label="Secondary tools and history">
                        ...
                        <div class="memorisation-tools-card-head">
                                ...
                                <button
                                    type="button"
                                    class="btn memorisation-section-toggle"
                                    @click="toggleMemorisationAdvancedSection('saved')"
                                    :aria-expanded="isMemorisationAdvancedSectionOpen('saved') ? 'true' : 'false'"
                        ...
                        <div class="modal-body session-history-modal-body">
                            ...
                            <div v-if="!sessionHistoryFilteredEntries.length" class="session-history-empty-state">
                                ...
                                <button
                                    ...
                                    @click="startNewMemorisationSessionSetup({ announce: true })">
      ...
      <div class="modal-content" :class="{ 'surat-dark-modal': isDarkTheme }">
        <div class="modal-header">
          <div>
            ...
            <p class="hifz-plan-modal-subtitle mb-0">Goal → Daily targets in seconds.</p>
          </div>
        ...
        <div class="modal-body">
          ...
          <section class="hifz-plan-wizard-section">
            <h6 class="hifz-plan-wizard-title">1) What is your target?</h6>
            
            <div class="hifz-plan-choice-grid">
              <button type="button" class="btn hifz-plan-choice-btn"
                :class="{ 'is-active': hifzWizard.targetType === 'whole-quran' }"
                @click="selectQuickTarget('whole-quran')">
                Whole Quran
              </button>
              <button type="button" class="btn hifz-plan-choice-btn"
                :class="{ 'is-active': hifzWizard.targetType === 'specific-juz' }"
                @click="selectQuickTarget('specific-juz')">
                One Juz
              </button>
              <button type="button" class="btn hifz-plan-choice-btn"
                :class="{ 'is-active': hifzWizard.targetType === 'specific-surah' }"
                @click="selectQuickTarget('specific-surah')">
                One Surah
              </button>
              <button type="button" class="btn hifz-plan-choice-btn"
            ...
            <!-- Conditional quick fields (minimal) -->
            <div v-if="hifzWizard.targetType === 'specific-juz'" class="hifz-plan-wizard-field mt-3">
              <label class="form-label">Which Juz?</label>
            ...
            <div v-if="hifzWizard.targetType === 'specific-surah'" class="hifz-plan-wizard-field mt-3">
              <label class="form-label">Which Surah?</label>
            ...
            <!-- Custom range stays but kept compact -->
            <div v-if="hifzWizard.targetType === 'custom-range'" class="hifz-plan-wizard-custom-range-grid mt-3">
              <!-- Your existing 4 fields: Start Surah/Ayah + End Surah/Ayah -->
              <!-- ... (keep as is, or make 2 rows) -->
            </div>
          </section>

          <!-- STEP 2: How Fast? (This is the magic for speed) -->
          <section class="hifz-plan-wizard-section">
            <h6 class="hifz-plan-wizard-title">2) How fast do you want to finish?</h6>
            
            <!-- Beginner / Advanced Presets -->
            <div class="hifz-plan-choice-grid mb-3">
              <button type="button" class="btn hifz-plan-choice-btn btn-lg"
                :class="{ 'is-active': hifzWizard.pace === 'beginner' }"
                @click="applyBeginnerPreset">
                🐢 Beginner<br><small>Gentle pace</small>
              </button>
              <button type="button" class="btn hifz-plan-choice-btn btn-lg"
                :class="{ 'is-active': hifzWizard.pace === 'balanced' }"
                @click="applyBalancedPreset">
                ⚖️ Balanced<br><small>Recommended</small>
              </button>
              <button type="button" class="btn hifz-plan-choice-btn btn-lg"
                :class="{ 'is-active': hifzWizard.pace === 'advanced' }"
                @click="applyAdvancedPreset">
                ⚡ Advanced<br><small>Intensive</small>
              </button>
            </div>

            <!-- Or manual deadline (still super simple) -->
            <div class="d-flex gap-3 align-items-end">
              <label class="flex-grow-1">
                <span class="form-label">Finish by</span>
                <select class="form-select" v-model="hifzWizard.deadlineType">
                  <option value="in-days">In X days</option>
                  <option value="by-date">Specific date</option>
                  <option value="by-ramadan">By next Ramadan</option>
                </select>
              </label>

              <div v-if="hifzWizard.deadlineType === 'in-days'" class="flex-grow-1">
                <span class="form-label">Days</span>
                <input type="number" min="7" max="3650" class="form-control" v-model.number="hifzWizard.deadlineDays">
              </div>

              <label v-if="hifzWizard.deadlineType === 'by-date'" class="flex-grow-1">
            ...
            <!-- Quick day presets -->
            <div class="hifz-plan-ratio-presets mt-2" v-if="hifzWizard.deadlineType === 'in-days'">
              <button class="btn hifz-plan-ratio-preset-btn" @click="hifzWizard.deadlineDays = 90">90 days</button>
              <button class="btn hifz-plan-ratio-preset-btn" @click="hifzWizard.deadlineDays = 180">6 months</button>
              <button class="btn hifz-plan-ratio-preset-btn" @click="hifzWizard.deadlineDays = 365">1 year</button>
            </div>
          </section>

          <!-- Advanced (collapsed by default) -->
          <div class="text-center my-3">
            <button type="button" class="btn hifz-plan-inline-toggle-btn"
              @click="isHifzWizardAdvancedOpen = !isHifzWizardAdvancedOpen">
              {{ isHifzWizardAdvancedOpen ? 'Hide' : 'Show' }} advanced options
              <i class="bi" :class="isHifzWizardAdvancedOpen ? 'bi-chevron-up' : 'bi-chevron-down'"></i>
            </button>
          </div>

          <div v-if="isHifzWizardAdvancedOpen" class="hifz-plan-advanced-stack">
            <!-- Your existing revision toggle, ratios, rest days, etc. – but much cleaner if possible -->
            <!-- Keep revision cadence, new/review ratio, rest days -->
          </div>

          <!-- Live Preview (always visible) -->
          <section class="hifz-plan-wizard-preview mt-4">
            <h6 class="hifz-plan-wizard-title">Your Plan Preview</h6>
            <div class="hifz-plan-preview-grid">
              ...
              <article class="hifz-plan-preview-card">
                <small>New ayahs / day</small>
                <strong>{{ hifzWizardPreview.dailyAyahs }}</strong>
              </article>
              <article class="hifz-plan-preview-card">
                <small>Finish by</small>
                <strong>{{ hifzWizardPreview.deadlineDateKey ? formatDateKey(hifzWizardPreview.deadlineDateKey) : '—' }}</strong>
              </article>
            </div>
            <p class="hifz-plan-preview-note mb-0 small">
              Daily goals + revision queue will sync automatically.
            </p>
            <p v-if="hifzPlanWizardError" class="text-danger small">{{ hifzPlanWizardError }}</p>
          </section>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn hifz-plan-modal-primary-btn"
									...
									<div
										...
										<button
                                            ...
                                            <button
                                                type="button"
                                                class="ayah-playlist-menu-item"
                                                @click.stop="openAyahTafsirModal(item)">
                                                <span class="ayah-actions-leading-label">
                                                    <i class="bi bi-journal-richtext" aria-hidden="true"></i>
                                                    <span>Tafseer</span>
                                                </span>
        ...
        <teleport to="body">
            <div
                ...
                :class="{
                    ...
                    'is-memorisation-tools-panel-open': isMemorisationToolbarVisible && isMemorisationOffcanvasVisible
                }"
