                            ...
                            <div class="section-header">
                                <div class="section-title">
                                    ...
                                    <div>
                                        ...
                                        <p class="section-desc">
                                            Preview what is inside each folder.
                                        </p>
                                    </div>
                                </div>
                                <div class="section-actions">
                                    <span class="section-hint"
                                        >Review or remove saved ayat</span
                                    >
                                    <button
                                    ...
                                    >
                                        Delete selected ({{
                                            selectedFoldersForDelete.length
                                        }})
                                    </button>
                                    <button
                                        type="button"
                                        class="btn section-toggle"
                                        @click="toggleSection('contents')"
                                        :aria-expanded="sectionOpen.contents"
                                    >
                                        <i
                                            class="fas"
                                            :class="
                                                sectionOpen.contents
                                                    ? 'fa-chevron-up'
                                                    : 'fa-chevron-down'
                                            "
                                        ></i>
                                ...
                                >
                                    <div>
                                        ...
                                        <div class="delete-note">
                                            Ayat in this folder will also be
                                            deleted.
                                        </div>
                                    </div>
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                ...
                                                                <option
                                                                    v-for="target in moveTargetFolders(
                                                                        folder
                                                                    )"
                                                                    :key="
                                                                        target.id
...
export default {
    ...
    computed: {
        feedbackClass() {
            return this.feedbackVariant === "danger"
                ? "alert-danger"
                : "alert-success";
        },
        selectedCount() {
    ...
    methods: {
        ...
        async ensureAuthenticated() {
            ...
            this.setFeedback(
                "Please log in to save bookmarks. Redirecting…",
                "danger"
            );
            clearTimeout(this.authRedirectTimer);
        ...
        async fetchCurrentBookmark() {
            const surahNumber = Number(
                this.ayah?.surah_number || this.ayah?.surah_id
            );
            const ayahNumber = Number(
                this.ayah?.ayah_number || this.ayah?.ayah_num
            );
            ...
            try {
                ...
                const bookmark = Array.isArray(response.data?.data)
                    ? response.data.data[0]
                    : null;
                this.currentBookmark = bookmark || null;
                if (bookmark?.folders) {
                    this.selectedFolderIds = bookmark.folders.map(
                        (folder) => folder.id
                    );
                }
                this.normalizeSelectedFolders();
        ...
        async createFolder() {
            ...
            if (
                this.folders.some(
                    (folder) =>
                        (folder.name || "").trim().toLowerCase() === nameKey
                )
            ) {
                this.setFeedback(
                    "Folder name already exists. Choose another.",
                    "danger"
                );
                return;
            }
            ...
            try {
                ...
                this.setFeedback(
                    apiMessage || "Failed to create folder.",
                    "danger"
                );
            } finally {
        ...
        async saveBookmark() {
            ...
            try {
                ...
                const existingIds =
                    this.currentBookmark?.folders?.map((folder) => folder.id) ||
                    [];
                const removeIds = existingIds.filter(
                    (id) => !selectedIds.includes(id)
                );
                const removableIds = removeIds.filter(
                    (id) => !this.isSmartFolder(id)
                );
                ...
                if (removableIds.length && this.currentBookmark?.id) {
                    try {
                        await Promise.all(
                            removableIds.map((id) =>
                                axios.delete(
                                    `/api/ayah-bookmarks/${this.currentBookmark.id}/folders/${id}`
                                )
                            )
                        );
        ...
        async confirmDelete() {
            ...
            const deletingMessage =
                ids.length === 1 ? "Deleting folder..." : "Deleting folders...";
            ...
            try {
                ...
                if (successfulDeletes.length > 0) {
                    ...
                    this.folders = this.folders.filter(
                        (folder) => !successfulDeletes.includes(folder.id)
                    );
                    ...
                    successfulDeletes.forEach((id) => {
                        const { [id]: removedExpanded, ...expanded } =
                            this.folderExpanded;
                        this.folderExpanded = expanded;
                        const { [id]: removedContents, ...contents } =
                            this.folderContents;
                ...
                if (failedDeletes.length === 0) {
                    ...
                    this.setFeedback(
                        "Failed to delete folders from backend.",
                        "danger"
                    );
                } else {
                    this.setFeedback(
                ...
                const errorMessage =
                    error.response?.data?.message ||
                    "Unable to delete folders from backend.";
        ...
        async removeAyahFromFolder(bookmark, folder) {
            ...
            try {
                ...
                await axios.delete(
                    `/api/ayah-bookmarks/${bookmark.id}/folders/${folder.id}`
                );
        ...
        moveTargetFolders(folder) {
            return this.folders.filter(
                (target) => !target.is_smart && target.id !== folder.id
            );
        },
        setIcon(icon) {
        ...
        async moveAyahToFolder(bookmark, fromFolder, event) {
            ...
            try {
                ...
                await axios.delete(
                    `/api/ayah-bookmarks/${bookmark.id}/folders/${fromFolder.id}`
                );

                const fromItems =
                    this.folderContents[fromFolder.id]?.items || [];
                this.folderContents = {
                    ...this.folderContents,
                    [fromFolder.id]: {
                        loading: false,
                        items: fromItems.filter(
                ...
                if (targetFolder) {
                    targetFolder.ayah_count =
                        (targetFolder.ayah_count || 0) + 1;
                    ...
                    if (this.folderContents[targetId]?.items) {
                        this.folderContents = {
                            ...
                            [targetId]: {
                                ...
                                items: [
                                    bookmark,
                                    ...this.folderContents[targetId].items,
                                ],
                            },
                        };
                    }
        ...
        selectAllFolders() {
            ...
            this.selectedFoldersForDelete = this.folders
                .filter((folder) => !folder.is_smart)
                .map((folder) => folder.id);
        },
        unselectAllFolders() {
...
.bookmark-modal {
    ...
    background: radial-gradient(
            120% 120% at 0% 0%,
            rgba(200, 155, 58, 0.12) 0%,
            transparent 45%
        ),
        radial-gradient(
            120% 120% at 100% 0%,
            rgba(15, 110, 99, 0.16) 0%,
            transparent 45%
        ),
